concrete IdiomHin of Idiom = CatHin ** open Prelude,Predef, ResHin,ParamX,CommonHindustani in {
  ...
  lin
    ...
    GenericCl vp = mkSClause "ko:I:" (agrP3 Masc Sg) vp ;
   ...
   ExistIP ip = 
     let cl = mkSClause ("vaha:n~" ++ ip.s ! Dir) (agrP3 ip.g ip.n) (predAux auxBe); 
	   in {
       s = \\t,p,qf => case qf of { 
	      QDir =>   cl.s ! t ! p ! ODir;
...
--    ImpPl1 vp = {s = "A:O:" ++ infVP True vp (agrP1 Masc Pl)} ;
      ImpPl1 vp = {s = "A:O:" ++ vp.obj.s ++ (vp.s !  VPTense VPFutr (agrP1 Masc Pl)).inf ++ vp.comp ! (agrP1 Masc Pl)} ;
